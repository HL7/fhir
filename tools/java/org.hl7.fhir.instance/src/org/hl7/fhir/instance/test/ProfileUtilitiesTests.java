...
//    compare("diagnosticreport-daf-dafdiagnosticreport.profile.xml", "diagnosticreport-qicore-qicore-diagnosticreport.profile.xml");
  ...
  public void testSnapshotGeneration() throws Exception {
  ...
  private void testSimple() throws EOperationOutcome, Exception {
    ...
    if (!ok) {
      ...
      throw new Exception("Snap shot generation simple test failed");
    } else 
  ...
  private void testCardinalityChange() throws EOperationOutcome, Exception {
    ...
    if (!ok) {
      compareXml(base, focus);
      throw new Exception("Snap shot generation chenge cardinality test failed");
    } else 
  ...
  private void testDocumentationAppend() throws EOperationOutcome, Exception {
    ...
    if (!ok) {
      compareXml(base, focus);
      throw new Exception("Snap shot generation documentation append failed");
    } else 
  ...
  private void textTypeNarrowing1() throws EOperationOutcome, Exception {
    ...
    if (!ok) {
      compareXml(base, focus);
      throw new Exception("Snap shot generation narrow type 1 failed");
    } else 
  ...
  private void textTypeNarrowing2() throws EOperationOutcome, Exception {
    ...
    if (!ok) {
      compareXml(base, focus);
      throw new Exception("Snap shot generation narrow type 2 failed");
    } else 
  ...
  private void testMapping() throws EOperationOutcome, Exception {
    ...
    if (!ok) {
      compareXml(base, focus);
      throw new Exception("Snap shot generation mapping changes failed");
    } else 
  ...
  private void testSlicingSimple() throws EOperationOutcome, Exception {
    ...
    if (!ok) {
      compareXml(base, focus);
      throw new Exception("Snap shot generation slicing failed");
    } else 
  ...
  private void testSlicingExtension(boolean implicit) throws EOperationOutcome, Exception {
    ...
    if (!ok) {
      compareXml(base, focus);
      throw new Exception("Snap shot generation slicing extensions simple ("+(implicit ? "implicit" : "not implicit")+") failed");
    } else 
  ...
  private void testSlicingExtensionComplex(boolean implicit) throws EOperationOutcome, Exception {
    ...
    if (!ok) {
      compareXml(base, focus);
      throw new Exception("Snap shot generation slicing extensions complex ("+(implicit ? "implicit" : "not implicit")+") failed");
    } else 
      System.out.println("Snap shot generation slicing extensions complex ("+(implicit ? "implicit" : "not implicit")+") passed");
  }

  private void testSlicingTask8742() throws Exception {
    StructureDefinition focus = new StructureDefinition();
    ...
    if (!ok) {
      compareXml(base, focus);
      throw new Exception("Snap shot generation test 8742 failed");
    } else 
      System.out.println("Snap shot generation test 8742 passed");
  }


  private boolean rule(boolean ok, String message) throws Exception {
    if (!ok)
      System.out.println("Test failed: " + message);
    return ok;
  }

  private void compareXml(StructureDefinition base, StructureDefinition focus) throws Exception {
    base.setText(null);
    focus.setText(null);
