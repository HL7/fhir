 ...
 */
  ...
  public void generateSnapshot(StructureDefinition base, StructureDefinition derived, String url, String profileName) throws Exception {
    ...
    if (snapshotStack.contains(derived.getUrl()))
      throw new Exception("Circular snapshot references detected; cannot generate snapshot (stack = "+snapshotStack.toString()+")");
          ...
          if (diffMatches.get(0).hasType() && diffMatches.get(0).getType().size() == 1 && diffMatches.get(0).getType().get(0).hasProfile() && !diffMatches.get(0).getType().get(0).getCode().equals("Reference")) {
            ...
            if (sd != null) {
              if (!sd.hasSnapshot()) {
                ...
                if (sdb == null)
                  throw new Exception("no base for "+sd.getBase());
...
//            tt.setUserData(DERIVATION_EQUALS, true);
  ...
  public XhtmlNode generateExtensionTable(String defFile, StructureDefinition ed, String imageFolder, boolean inlineGraphics, boolean full, String corePath) throws Exception {
  ...
  private Cell genTypes(HierarchicalTableGenerator gen, Row r, ElementDefinition e, String profileBaseFileName, StructureDefinition profile, String corePath) throws Exception {
    Cell c = gen.new Cell();
    r.getCells().add(c);
  ...
  public XhtmlNode generateTable(String defFile, StructureDefinition profile, boolean diff, String imageFolder, boolean inlineGraphics, String profileBaseFileName, boolean snapshot, String corePath) throws Exception {
  ...
  private void genElement(String defPath, HierarchicalTableGenerator gen, List<Row> rows, ElementDefinition element, List<ElementDefinition> all, List<StructureDefinition> profiles, boolean showMissing, String profileBaseFileName, Boolean extensions, boolean snapshot, String corePath) throws Exception {
  ...
  private boolean extensionIsComplex(String value) throws EOperationOutcome, Exception {
    if (value.contains("#")) {
      StructureDefinition ext = context.fetchResource(StructureDefinition.class, value.substring(0, value.indexOf("#")));
      if (ext == null)
        return false;
...
//  private static String listStructures(StructureDefinition p) throws Exception {
//    StringBuilder b = new StringBuilder();
//    boolean first = true;
...
//        throw new Exception(b.toString());
  ...
  public void sortDifferential(StructureDefinition base, StructureDefinition diff, String name, List<String> errors) throws EOperationOutcome, Exception {
