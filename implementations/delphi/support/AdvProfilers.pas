...
//
// NOTES
//  This hash table implementation doesn't descend from TAdvHash because it
//  is used for tracking all objects instances in the AdvLibrary system.  That is,
//  we can't use hashable instances to track the lifetime of other instances as it
//  is a circular relationship.
//
//  Instances tracked in the system can have the following attributes flagged.
//   * Breakpoint - flagged as invoking a breakpoint when accessed at specified points.
//     (eg. a particular instance in BeforeDestruction).
//   * Permanent - flagged as a global object that is never freed by the system.
//     (these objects will not be reported by the profiler as being leaks).
//
...
Uses
  MemorySupport, StringSupport, DateSupport, NetworkSupport, HashSupport,
