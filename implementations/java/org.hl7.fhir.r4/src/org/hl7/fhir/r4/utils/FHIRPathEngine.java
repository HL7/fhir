    ...
    StructureDefinition sd = worker.fetchResource(StructureDefinition.class, context);
	  ...
	  if (ed.fixedType != null) 
		...
		for (TypeRefComponent t : ed.getDefinition().getType()) 
		  types.addType(t.getWorkingCode());
	  }
	}
...
//  return result;
    ...
    if (m != null && hasDataType(m.definition)) {
      if (m.fixedType != null)
      {
        ...
        for (TypeRefComponent t : m.definition.getType()) {
          StructureDefinition dt = worker.fetchResource(StructureDefinition.class, ProfileUtilities.sdNs(t.getWorkingCode(), worker.getOverrideVersionNs()));
          if (dt == null)
            throw new DefinitionException("unknown data type "+t.getWorkingCode());
          sdl.add(dt);
        }
    } else {
      sdl.add(sd);
      ...
      if (name.equals("**")) {
          ...
          if (ed.getPath().startsWith(path))
            for (TypeRefComponent t : ed.getType()) {
              if (t.hasCode() && t.getCodeElement().hasValue()) {
                String tn = null;
                if (t.getWorkingCode().equals("Element") || t.getWorkingCode().equals("BackboneElement"))
                  tn = sdi.getType()+"#"+ed.getPath();
                else
                  tn = t.getWorkingCode();
                if (t.getWorkingCode().equals("Resource")) {
        ...
        for (ElementDefinition ed : sdi.getSnapshot().getElement()) {
          if (ed.getPath().startsWith(path) && !ed.getPath().substring(path.length()).contains("."))
            for (TypeRefComponent t : ed.getType()) {
              if (Utilities.noString(t.getWorkingCode())) // Element.id or Extension.url
                result.addType("System.string");
              else if (t.getWorkingCode().equals("Element") || t.getWorkingCode().equals("BackboneElement"))
                result.addType(sdi.getType()+"#"+ed.getPath());
              else if (t.getWorkingCode().equals("Resource"))
                result.addTypes(worker.getResourceNames());
              else
                result.addType(t.getWorkingCode());
            }
        }
      } else {
        ...
        if (ed != null) {
          if (!Utilities.noString(ed.getFixedType()))
            ...
            for (TypeRefComponent t : ed.getDefinition().getType()) {
              if (Utilities.noString(t.getWorkingCode())) {
  ...
  private ElementDefinitionMatch getElementDefinition(StructureDefinition sd, String path, boolean allowTypedName) throws PathEngineException {
    for (ElementDefinition ed : sd.getSnapshot().getElement()) {
      ...
      if (ed.getPath().contains(".") && path.startsWith(ed.getPath()+".") && (ed.getType().size() > 0) && !isAbstractType(ed.getType())) { 
        ...
        StructureDefinition nsd = worker.fetchResource(StructureDefinition.class, ProfileUtilities.sdNs(ed.getType().get(0).getWorkingCode(), worker.getOverrideVersionNs()));
  	    if (nsd == null) 
  	      throw new PathEngineException("Unknown type "+ed.getType().get(0).getWorkingCode());
  ...
  private boolean isAbstractType(List<TypeRefComponent> list) {
	return list.size() != 1 ? true : Utilities.existsInList(list.get(0).getWorkingCode(), "Element", "BackboneElement", "Resource", "DomainResource");
}


  private boolean hasType(ElementDefinition ed, String s) {
    for (TypeRefComponent t : ed.getType()) 
      if (s.equalsIgnoreCase(t.getWorkingCode()))
        return true;
    return false;
  }

  private boolean hasDataType(ElementDefinition ed) {
    return ed.hasType() && !(ed.getType().get(0).getWorkingCode().equals("Element") || ed.getType().get(0).getWorkingCode().equals("BackboneElement"));
  }
    ...
    if (expr.getKind() == Kind.Name) {
      ...
      if ("resolve".equals(expr.getName())) {
        ...
        if (!element.getType().get(0).hasTarget())
          throw new DefinitionException("illegal use of resolve() in discriminator - type on "+element.getId()+" is not Reference ("+element.getType().get(0).getWorkingCode()+")");
...
//        targetUrl = targetUrl.substring(1,targetUrl.length()-1);
  ...
  private boolean tailMatches(ElementDefinition t, String d) {
    ...
    if (d.contains("["))
      ...
      return true;
    else if (t.getType().size() == 1 && t.getType().get(0).getWorkingCode() != null && t.getPath() != null && t.getPath().toUpperCase().endsWith(t.getType().get(0).getWorkingCode().toUpperCase()))
